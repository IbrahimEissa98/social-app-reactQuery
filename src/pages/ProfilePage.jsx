...
export default function ProfilePage() {
  ...
  const { userPosts, deleteComment, deletePost } = useApis();
  ...
  if (isError) {
    if (error.message == "Network Error") {
      ...
      addToast({
        title: `Error`,
        description: `Invalid to get your data! try again`,
        color: "danger",
      });
    }
  }
  ...
  return (
    <>
      {isLoading ? (
        <LoadingComponent title={"profile"} />
      ) : (
        <>
          {isFetching && (
          ...
          <ProfileInfoCard
            user={user}
            onOpenPhotoPasswordModal={onOpenPhotoPasswordModal}
          ...
          <div className="grid gap-2 pb-5 w-full! mt-5">
            <>
              ...
              {profilePosts.length === 0 && (
                <h2 className="font-bold text-3xl text-danger text-center my-5">
                  No Posts Found
                </h2>
              )}
            ...
            <CreatePostBox
              ...
              getPosts={() => queryClient.invalidateQueries(["profilePosts"])}
              isOpen={isOpenCreatePost}
              onClose={onCloseCreatePost}
